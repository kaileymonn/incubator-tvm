...
public:
	...
	Expr MakeFunc(
		...
		if (compiler_!="") {
			...
			if (func_name_!="") {
				//Set defined function name
				func = WithAttr(std::move(func), attr::kComposite, String(GetName(func_name_)));
			} else if (func_name_=="") {
				// Auto set function name.
				func = WithAttr(std::move(func), attr::kComposite, String(GetName(subgraph->func_name.str())));
			} else {
				LOG(WARNING)
						<< "The type of func name must be string.";
			}
