...
import onnx
from onnx import numpy_helper, helper, external_data_helper
...
class BatchNorm(ToOnnxOpConverter):
    ...
    def _impl_v7(cls, graph, inputs, attrs={}, args=None):
        assert attrs.setdefault('scale', 1) == True and attrs.setdefault('center', 1) == True, 'Scale(gamma) and bias(beta) of BatchNorm with opset > 7 must be reserved.'
        return cls.convert(graph, inputs, attrs)
...
class TopK(ToOnnxOpConverter):
    """ Operator converter for topk op."""
    name = 'TopK'
    @classmethod
    def _impl_v1(cls, graph, inputs, attrs={}, args=None):
        ignores=['ret_type','is_ascend','dtype']
        topk = AttrCvt(cls.name, ignores=ignores, extras=extras, counts_out=2)(graph, inputs, attrs)
        ret_type = attrs.get('ret_type', 'both')
        if ret_type == 'both':
            return topk
    ...
    @classmethod
    def _impl_v10(cls, graph, inputs, attrs={}, args=None):
        ignores=['ret_type','is_ascend','dtype','k']
        make_const = Constant.get_impl(cls.opset)
        k = make_const(graph, 'k', attrs.get('k',1))
        topk = AttrCvt(cls.name, ignores=ignores, counts_out=2)(graph, inputs + k, attrs)
        ret_type = attrs.get('ret_type', 'both')
        if ret_type == 'both':
            return topk
        elif ret_type == 'values':
    ...
    def _impl_v11(cls, graph, inputs, attrs={}, args=None):
        ignores=['ret_type','is_ascend','dtype','k']
        extras={'largest':1, 
                'sorted': 1 if attrs.get('is_ascend', 0) == 0 else 0
        }
        
        make_const = Constant.get_impl(cls.opset)
        k = make_const(graph, 'k', attrs.get('k',1))
        topk = AttrCvt(cls.name, ignores=ignores, extras=extras, counts_out=2)(graph, inputs + k, attrs)
        ret_type = attrs.get('ret_type', 'both')
        if ret_type == 'both':
            return topk
        elif ret_type == 'values':
...
class ONNXGraph():
    ...
    def __init__(self, params, opset, graph_name, doc_string):
        ...
        self.ret_types = None
        self.make_const = Constant.get_impl(self.opset)
    ...
    def add_const(self, const):
        ...
        return self.make_const(self,'const',data)
    def add_nodes(self, c, op_name, inputs, attrs):
        outputs = self.convert_op(op_name, inputs, attrs, c.args)
        self.final_outputs = outputs
        ret_types = c.checked_type
        self.ret_types = [ret_types] if isinstance(ret_types, TensorType) else ret_types.fields
        return outputs
    ...
    def make_model(self):
        for i, output in enumerate(self.final_outputs):
            node = make_tensor_info(output, self.ret_types[i])
...
def to_onnx(model, params={}, graph_name="", opset = None, doc_string=None):
    ...
    if isinstance(model, tvm.IRModule):
        ...
        model = tvm.IRModule.from_expr(model)
    else:
        raise TypeError(
...
def get_onnx_bytes(model):
    if isinstance(model, bytes):
        model = _deserialize(model, ModelProto())
    import io, codecs
    f = io.BytesIO()
    for tensor in external_data_helper._get_all_tensors(model):
        if external_data_helper.uses_external_data(tensor):
            save_external_data(tensor, f)
